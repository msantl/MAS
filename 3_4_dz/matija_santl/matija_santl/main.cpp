...
image* load_image(const char *input) {
	FILE *f;
	image* img;
    /* load .ppm file */
    f = fopen(input, "rb");
...
void output_block(ycbcr_block_int res, const char *output) {
	FILE *f;
	f = fopen(output, "w");
...
void intel_DCT(ycbcr_block* in_blk,
         ycbcr_block* out_blk) {
    int i, j, k, u, v;
    double sum, Cu, Cv;
    ...
    for (k = 0; k < 3; ++k) {
		...
		for (u = 0; u < BLOCK_SIZE; ++u) {
			for (v = 0; v < BLOCK_SIZE; ++v) {
				x[8 * u + v] = in_blk->colors[u][v].ycbcr[k];	
			}
		}

		IppStatus status = ippiDCT8x8Fwd_32f_C1I( x );

        for (u = 0; u < BLOCK_SIZE; ++u) {
            for (v = 0; v < BLOCK_SIZE; ++v) {
				out_blk->colors[u][v].ycbcr[k] = x[8 * u + v];
            }
        }
    }
    return;
